...
CONTRACT_ARTICLES = [
    ...
    ("第4条（指揮命令）",
     "甲は、乙の従業員に対して直接の指揮命令を行わないものとする。"
     "乙の従業員に対する業務上の指示は、乙の責任者を通じて行うものとする。"
     "甲の委託元その他の第三者が乙の従業員に対し直接の指揮命令を行うことのないよう、甲は必要な措置を講じるものとする。"),
    ("第5条（再委託の禁止）",
     "乙は、甲の書面による事前承諾なく、委託業務の全部または一部を第三者に再委託してはならない。"),
    ("第6条（秘密保持）",
     "甲および乙は、本契約に関連して知り得た相手方の技術上、営業上の秘密を、"
     "相手方の事前の書面による承諾なく第三者に開示・漏洩してはならない。"
     "秘密保持義務は、本契約の終了後もなお存続するものとし、"
     "その期間は甲の委託元との契約に定める秘密保持期間に準じるものとする。"
     "ただし、委託元との契約に定めがない場合は、契約終了後3年間とする。"),
    ("第7条（個人情報の取扱い）",
     "乙は、本契約に基づく業務の遂行にあたり知り得た個人情報を、"
     "個人情報の保護に関する法律その他関連法令に従い、適切に管理するものとする。"
     "乙は、業務の終了または契約の解除後、速やかに個人情報を返還または廃棄し、"
     "甲の求めに応じてその旨を証明するものとする。"),
    ("第8条（報酬の支払い）",
     "甲は、乙に対し、個別の注文書に定める報酬を、乙の請求に基づき支払うものとする。"),
    ("第9条（損害賠償）",
     "甲または乙は、本契約または個別の注文書に違反して相手方に損害を与えた場合、"
     "直接かつ現実に生じた損害を賠償する責任を負う。"
     "ただし、賠償額は、当該損害の原因となった個別注文書に定める報酬月額の3ヶ月分を上限とする。"),
    ("第10条（契約期間）",
...
def generate_contract_pdf(partner, signed_at=None, created_date=None):
    ...
    if company:
        ...
        p.setFont(font_name, 11)
        p.drawString(20 * mm, y_pos - 5 * mm, company.name if company else '')
    ...
    if company:
        ...
        p.drawString(20 * mm, y_pos - 7 * mm, company.name if company else '')
